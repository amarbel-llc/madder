	...
	utility.EnvVars = append(utility.EnvVars,
		...
		futility.EnvVar{
			...
			Description: "Set to a truthy value (\"1\", \"true\", \"yes\", " +
				...
				".madder/ directory. With this flag set, cutting-garden uses " +
				"standard XDG resolution only ($XDG_DATA_HOME etc., " +
				"defaulting to $HOME/.local/share). Useful for embedders and " +
				"test harnesses that exec cutting-garden from a cwd whose " +
				"path branch a MADDER_CEILING_DIRECTORIES entry can't gate.",
		},
		futility.EnvVar{
			Name: "HOME",
