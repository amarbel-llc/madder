...
// paramHasCompleter reports whether the param exposes a flagCompleter with a
// non-nil value — i.e. it wants dynamic __complete dispatch.
func paramHasCompleter(p Param) bool {
	fc, ok := p.(interface{ flagCompleter() ParamCompleter })
	if !ok {
		return false
	}
	return fc.flagCompleter() != nil
}
...
func (u *Utility) generateBashCompletion(dir string) error {
	...
	for _, c := range cmds {
		...
		// positionalCompletable: non-Bool params with a Completer, in
		// declaration order. Mirrors the positional assignment logic in cli.go.
			...
			if len(completableParams) > 0 {
				...
				for _, p := range completableParams {
					...
					fmt.Fprintf(&b, "                    COMPREPLY=( $(compgen -W \"$(%s __complete --command %s --param %s)\" -- \"${cur}\") )\n",
						u.Name, c.name, p.paramName())
					fmt.Fprintf(&b, "                    ;;\n")
				}
				fmt.Fprintf(&b, "                *)\n")
				if len(positionalCompletable) > 0 {
					u.emitBashPositionalCompletions(&b, c.name, c.cmd.Params, positionalCompletable, flagForms)
				} else {
					fmt.Fprintf(&b, "                    COMPREPLY=( $(compgen -W %q -- \"${cur}\") )\n", strings.Join(flagForms, " "))
				}
				fmt.Fprintf(&b, "                    ;;\n")
				fmt.Fprintf(&b, "            esac\n")
			} else {
...
// positional param. Mirrors the positional assignment logic in cli.go:
...
func (u *Utility) emitBashPositionalCompletions(
	...
	for _, e := range entries {
		fmt.Fprintf(b, "                        %d)\n", e.index)
		fmt.Fprintf(b, "                            COMPREPLY=( $(compgen -W \"$(%s __complete --command %s --param %s)\" -- \"${cur}\") )\n",
			u.Name, cmdName, e.param.paramName())
		fmt.Fprintf(b, "                            ;;\n")
	}
	fmt.Fprintf(b, "                        *)\n")
	fmt.Fprintf(b, "                            COMPREPLY=( $(compgen -W %q -- \"${cur}\") )\n", strings.Join(flagForms, " "))
...
func (u *Utility) emitFishParamCompletions(b *strings.Builder, cmdName string, cmds []sortedCommand) {
	for _, c := range cmds {
		for _, p := range c.cmd.Params {
			...
			desc := strings.ReplaceAll(p.paramDescription(), "'", "\\'")
			shortOpt := ""
			if p.paramShort() != 0 {
				shortOpt = fmt.Sprintf(" -s %c", p.paramShort())
			}
			completerArg := ""
			if paramHasCompleter(p) {
			...
			fmt.Fprintf(b, "complete -c %s -n '__fish_seen_subcommand_from %s' -l %s%s -d '%s'%s\n",
				cmdName, c.name, p.paramName(), shortOpt, desc, completerArg)
		}
		// Positional completions: for non-Bool params with Completer, add a
		// rule that fires when the flag form hasn't been used.
		for _, p := range c.cmd.Params {
			if p.jsonSchemaType() == "boolean" || !paramHasCompleter(p) {
				continue
			}
			desc := strings.ReplaceAll(p.paramDescription(), "'", "\\'")
			fmt.Fprintf(b, "complete -c %s -n '__fish_seen_subcommand_from %s; and not __fish_contains_opt %s' -ra '(%s __complete --command %s --param %s)' -d '%s'\n",
				cmdName, c.name, p.paramName(), u.Name, c.name, p.paramName(), desc)
		}
	}
}
